"""
Generates a LaTeX research paper describing the Bank Fragility replication
project and compiles it to PDF. Output saved to documents/.
"""
...
TEX_PATH = DOCUMENTS / "project_description.tex"
PDF_PATH = DOCUMENTS / "project_description.pdf"
...
\begin{titlepage}
    ...
    \vspace{2cm}
    {\normalsize
    \textbf{Report Date:} """ + REPORT_DATE_FMT + r""" \\[0.3em]
    \textbf{Shock Window:} """ + MARKET_START_DATE + r""" to """ + MARKET_END_DATE + r""" \\[0.3em]
...
For this replication we use the Call Report as of """ + REPORT_DATE_FMT + r""".
...
of """ + RMBS_MULTIPLIER + r""" is applied to account for convexity risk --- the
...
(""" + MARKET_START_DATE + r""" to """ + MARKET_END_DATE + r""").
...
\begin{itemize}[noitemsep]
    ...
    \item $m_k$ is the asset-class multiplier: $m_k = """ + RMBS_MULTIPLIER + r"""$ for RMBS
...
(""" + MARKET_START_DATE + r""" to """ + MARKET_END_DATE + r""") is computed by
...
are more exposed. RMBS receive an additional multiplier of """ + RMBS_MULTIPLIER + r"""
...
\midrule
\texttt{REPORT\_DATE\_SLASH} & """ + REPORT_DATE_FMT + r""" & FFIEC reporting quarter \\
\texttt{REPORT\_DATE}        & """ + REPORT_DATE + r"""     & Date in filename format \\
\texttt{MARKET\_START\_DATE} & """ + MARKET_START_DATE + r""" & Start of shock window \\
\texttt{MARKET\_END\_DATE}   & """ + MARKET_END_DATE + r"""   & End of shock window \\
\texttt{RMBS\_MULTIPLIER}    & """ + RMBS_MULTIPLIER + r"""   & Convexity haircut for MBS \\
