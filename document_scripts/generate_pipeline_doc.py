"""
Generates a LaTeX document describing the Bank Fragility pipeline
and compiles it to PDF. Output saved to documents/.
"""
...
REPORT_DATE_FMT   = REPORT_DATE_SLASH
MARKET_START_DATE = "2020-01-01"
MARKET_END_DATE   = f"{REPORT_DATE[4:]}-{REPORT_DATE[:2]}-{REPORT_DATE[2:4]}"
RMBS_MULTIPLIER   = os.getenv("RMBS_MULTIPLIER",   "1.25")

TEX_PATH = DOCUMENTS / "pipeline_documentation.tex"
PDF_PATH = DOCUMENTS / "pipeline_documentation.pdf"
...
\begin{titlepage}
    ...
    \vspace{2cm}
    {\normalsize
    \textbf{Report Date:} """ + REPORT_DATE_FMT + r""" \\[0.4em]
    \textbf{Shock Window:} """ + MARKET_START_DATE + r""" to """ + MARKET_END_DATE + r""" \\[0.4em]
    \textbf{RMBS Multiplier:} """ + RMBS_MULTIPLIER + r"""
    \par}
    \vfill
...
\textbf{Configuration:}
\begin{lstlisting}
REPORT_DATE_SLASH = """ + REPORT_DATE_FMT + r"""   # date shown on FFIEC website
REPORT_DATE       = """ + REPORT_DATE + r"""         # date in zip filename
\end{lstlisting}

\textbf{Output:}
\begin{lstlisting}
_data/FFIEC CDR Call Bulk All Schedules """ + REPORT_DATE + r""".zip
\end{lstlisting}
...
\textbf{Outputs:}
\begin{lstlisting}
_data/bank_panel_""" + REPORT_DATE + r""".parquet
_output/summary_stats_""" + REPORT_DATE + r""".xlsx
_output/figure_A1_""" + REPORT_DATE + r""".png
...
\textbf{Configuration:}
\begin{lstlisting}
MARKET_START_DATE = """ + MARKET_START_DATE + r"""
MARKET_END_DATE   = """ + MARKET_END_DATE + r"""
RMBS_MULTIPLIER   = """ + RMBS_MULTIPLIER + r"""
\end{lstlisting}
...
\midrule
\texttt{REPORT\_DATE\_SLASH} & """ + REPORT_DATE_FMT + r""" & \texttt{pull\_ffiec.py} \\
\texttt{REPORT\_DATE}        & """ + REPORT_DATE + r"""     & \texttt{process\_ffiec.py}, \texttt{make\_table\_1.py} \\
\texttt{MARKET\_START\_DATE} & """ + MARKET_START_DATE + r""" & \texttt{compute\_yield\_shocks.py} \\
\texttt{MARKET\_END\_DATE}   & """ + MARKET_END_DATE + r"""   & \texttt{compute\_yield\_shocks.py} \\
\texttt{RMBS\_MULTIPLIER}    & """ + RMBS_MULTIPLIER + r"""   & \texttt{compute\_yield\_shocks.py}, \texttt{make\_table\_1.py} \\
...
def main() -> None:
    ...
    print(f"Writing {TEX_PATH.name}...")
    TEX_PATH.write_text(LATEX, encoding="utf-8")

    print("Compiling to PDF (running pdflatex twice for TOC)...")
    for run_num in range(1, 3):
        result = subprocess.run(
            [
                "pdflatex",
                "-interaction=nonstopmode",
                "-output-directory", str(DOCUMENTS),
                str(TEX_PATH),
            ],
            capture_output=True,
            text=True,
        ...
        if not PDF_PATH.exists() or "Fatal error" in result.stdout:
            print(f"pdflatex failed on run {run_num}:")
            print(result.stdout[-3000:])
            sys.exit(1)
        print(f"  Run {run_num}/2 done.")
